...
const featuredEvents: FeaturedEvent[] = [
  {
    ...
    details: { guests: '500+', duration: '3 Days', location: 'Banglore Palace' },
    category: 'Wedding',
  },
  {
    id: 2,
    ...
    details: { guests: '300+', duration: '2 Days', location: 'Convention Center' },
    category: 'Corporate',
  },
  {
    id: 3,
  ...
  },
  {
    ...
    details: { guests: '100+', duration: '4 Hours', location: 'Community Hall' },
    category: 'Kids',
  },
];

// Memoized Slide Component
const Slide = memo(({ event, imageSrc }: { event: FeaturedEvent; imageSrc: string | null }) => {
  const navigate = useNavigate();
  if (!imageSrc) return <div className="w-full h-full bg-gray-200 rounded-3xl" />;
    ...
    >
      <div className="grid lg:grid-cols-2 h-full">
        ...
        <div className="relative overflow-hidden h-64 sm:h-96 lg:h-full">
          <img src={imageSrc} alt={event.title} className="w-full h-full object-cover rounded-l-3xl" />
          <div className="absolute inset-0 bg-gradient-to-r from-black/20 to-transparent rounded-l-3xl" />
          {/* Category Badge */}
          <div className="absolute top-4 left-4">
            <span className="bg-yellow-400 text-red-900 px-4 py-2 rounded-full text-sm font-semibold">{event.category}</span>
          </div>
          {/* Elegant Title */}
        ...
        <div className="bg-white p-6 sm:p-10 lg:p-12 flex flex-col justify-center rounded-r-3xl">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
            <p className="text-lg text-yellow-600 font-semibold mb-4 sm:mb-6">{event.subtitle}</p>
            <p className="text-gray-600 text-base sm:text-lg leading-relaxed mb-6 sm:mb-8">{event.description}</p>
            ...
            <div className="grid grid-cols-3 gap-4 text-center mb-6">
              <div>
                ...
                <p className="font-semibold text-red-900">{event.details.guests}</p>
              </div>
              <div>
                <div className="bg-red-100 w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2">
                ...
                <p className="font-semibold text-red-900">{event.details.duration}</p>
              </div>
              <div>
                <div className="bg-red-100 w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2">
                ...
                <p className="font-semibold text-red-900">{event.details.location}</p>
              </div>
            </div>
...
const FeaturedEventsCarousel: React.FC = () => {
  ...
  const [images, setImages] = useState<(string | null)[]>(Array(featuredEvents.length).fill(null));
  ...
  useEffect(() => {
    ...
    featuredEvents[currentSlide].image().then(module => {
      if (isMounted) setImages(prev => { const arr = [...prev]; arr[currentSlide] = module.default; return arr; });
    });
    const nextIndex = (currentSlide + 1) % featuredEvents.length;
    featuredEvents[nextIndex].image().then(module => setImages(prev => { const arr = [...prev]; arr[nextIndex] = module.default; return arr; }));
    return () => { isMounted = false; };
  }, [currentSlide]);

  // Auto-slide
  useEffect(() => {
    const interval = setInterval(() => setCurrentSlide(prev => (prev + 1) % featuredEvents.length), 6000);
  ...
  return (
    <section ref={carouselRef} className="py-20 bg-gradient-to-br from-red-50 to-red-100">
      <div className="container mx-auto px-4">
        ...
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
          <h2 className="text-4xl md:text-5xl font-bold text-red-900 mb-4">Featured Events</h2>
          <p className="text-xl text-red-700 max-w-3xl mx-auto">Discover our most spectacular celebrations and see how we bring dreams to life</p>
        </motion.div>
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="relative h-[600px] lg:h-[500px] rounded-3xl overflow-hidden shadow-2xl">
            <AnimatePresence mode="wait">
              <Slide event={featuredEvents[currentSlide]} imageSrc={images[currentSlide]} />
            </AnimatePresence>

            {/* Navigation */}
            <button onClick={prevSlide} className="absolute left-3 top-1/2 transform -translate-y-1/2 bg-white/90 hover:bg-white backdrop-blur-sm text-red-900 p-2 rounded-full shadow-lg transition-all duration-300 z-10">
              <ChevronLeft size={22} />
            </button>
            <button onClick={nextSlide} className="absolute right-3 top-1/2 transform -translate-y-1/2 bg-white/90 hover:bg-white backdrop-blur-sm text-red-900 p-2 rounded-full shadow-lg transition-all duration-300 z-10">
          ...
          <div className="flex justify-center mt-6 space-x-2">
            {featuredEvents.map((_, index) => (
              <button
                ...
                className={`transition-all duration-300 rounded-full ${index === currentSlide ? 'w-10 h-2 bg-red-800' : 'w-2 h-2 bg-red-300 hover:bg-red-500'}`}
              />
            ))}
          </div>
        </div>
