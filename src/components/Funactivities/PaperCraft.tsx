  ...
  return (
    <section className="py-20 min-h-screen bg-gradient-to-br from-pink-100 via-yellow-100 to-green-100">
      <div className="container mx-auto px-4 relative">
        ...
        <motion.h2
          className="text-4xl md:text-5xl font-extrabold text-center text-pink-700 mb-4 tracking-tight drop-shadow relative z-10"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 max-w-6xl mx-auto relative z-10">
          {PaperCraftImages.map((img: string, idx: number) => (
            ...
            >
              <div className="rounded-3xl overflow-hidden shadow-2xl bg-white/60 backdrop-blur-lg border border-white/30 transition-all duration-300 group-hover:shadow-pink-200">
                <img
                  src={img}
                  alt={`Paper Craft ${idx + 1}`}
                ...
                {/* Stylish badge */}
                <span className="absolute top-4 right-4 bg-gradient-to-r from-pink-500 to-green-400 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg backdrop-blur-sm">
                  Trending
                </span>
              </div>
            </motion.div>
