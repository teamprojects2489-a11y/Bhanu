...
import { PotMakingImages } from '../../assets/index';

const PotMaking: React.FC = () => {
  const [bgImg, setBgImg] = useState<string | null>(null);

  return (
    <section className="py-20 min-h-screen bg-gradient-to-br from-amber-100 via-orange-100 to-yellow-100">
          ...
          {PotMakingImages.map((img: string, idx: number) => (
            ...
            >
              <div className="rounded-3xl overflow-hidden shadow-2xl bg-white/60 backdrop-blur-lg border border-white/30 transition-all duration-300 group-hover:shadow-amber-200">
                <img
                  src={img}
                  alt={`Pot Making ${idx + 1}`}
                  className="w-full h-72 object-cover transition-transform duration-300 group-hover:scale-105"
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-amber-400/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                  <span className="text-white font-bold text-lg drop-shadow-lg tracking-wide">
                    Pot Making {idx + 1}
                  </span>
                </div>
                {/* Stylish badge */}
                <span className="absolute top-4 right-4 bg-gradient-to-r from-amber-500 to-orange-400 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg backdrop-blur-sm">
                  Trending
                </span>
              </div>
            </motion.div>
